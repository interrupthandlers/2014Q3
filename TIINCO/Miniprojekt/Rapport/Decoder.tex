...
\code{md.decodeFullStep()} Runs remaining decoding iterations -- a single decoding iteration can be performed using \code{md.decodeSingleStep()}. This allows monitoring of the class data properties such as the buffer and syndrome registers between iterations during the decoding.

\section{Generation of syndrome vectors}
Because the Meggitt decoder only corrects the most significant bit at a given time, the Meggitt decoder only needs to detect syndome vectors, which correspond to an error pattern with the highest bit in error. The function \code{createSyndromeVectors()} generates a matrix, S, containing the relevant syndrome vectors, when an instance of the \code{MeggittDecoderImpl} class is created. The part of the function relevant for a code capable of correcting 2 errors is listed (\ref{lst:CreateSyndromes}). \code{MD} is the instance of the decoder class.
